...
console.log("Fork of Unreal Engine %UE_VERSION% with HTML5 ES3 (WebGL 2) platform plugin - see https://github.com/SpeculativeCoder/UnrealEngine-HTML5-ES3 for more information.");
...
//定义一个全局对象 UE 用于与 JavaScript 代码进行通信
var UE = UE || {};

UE.interface = {
	ReceiveUEMsg: function(TypeString,listener) {
		if (TypeString != undefined && listener != undefined ) {
			//使用字符串创建自定义事件，TypeString.toString()，null 和 undefined 没有该方法
			window.addEventListener(String(TypeString), function(e) {
				console.log("UE4_init message: " + e.detail.message);
	...
	SendUE: function(msg) {
		...
		// 使用 TextEncoder 对字符串进行 UTF-8 编码
		var strBuff = new TextEncoder().encode(msg);

		// 为字符串分配内存，额外加 1 用于存储字符串结束符 '\0'
		const buff = Module._malloc(strBuff.length + 1);

		// 获取 WebAssembly 内存视图
		Module.HEAPU8.set(strBuff,buff);

		// 手动添加字符串结束符 '\0'
		Module.HEAPU8[buff + strBuff.length] = 0;

		// 调用 WebAssembly 模块中的 _sendue 函数
		Module._sendue(buff);
		...
		// 打印日志信息
		console.log("UE4_sendUE: " + msg);
	}
};

window.UE =UE;

// emscripten 加载完成后调用此函数
Module.onRuntimeInitialized = function() {

	UE.interface.SendUE("nini");
	console.log("UE4_init");
/*	setTimeout(()=>{
		console.log("UE4_init done");
		//Module.ccall('sendue',null,['string'],['你好']);
		UE.interface.SendUE("你好");
	},5000)*/
...
//接收来自 UE4 的消息
UE.interface.ReceiveUEMsg("UEmessage", function(msg) {
	console.log("UE4_init message: " + msg);
});
...
function resizeCanvas(aboutToEnterFullscreen) {
	...
	var cssHeight = Math.max(minimumCanvasHeightCssPixels, mainAreaRect.bottom - mainAreaRect.top, window.innerHeight * minimumCanvasHeightFractionOfBrowserWindowHeight - (%OVERLAY_TOOLBAR% ? 0 : buttonAreaRect.height));
...
function reportDataBytesStoredInIndexedDB(deltaBytes) {
	...
	document.getElementById('clear_indexeddb').title = '%CLEAR_INDEXEDDB_BUTTON_TEXT% (' + formatBytes(Module['dataBytesStoredInIndexedDB']) + ')';
}
