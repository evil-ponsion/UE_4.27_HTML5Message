...
var UE_JavascriptLibrary = {
  ...
  UE_GetCurrentCultureName: function (address, outsize) {
    ...
    if (culture_name.length >= outsize)
      return 0;
    Module.stringToAscii(culture_name, address);
    return 1;
  },
  ...
  UE_MakeHTTPDataRequest: function (ctx, url, verb, payload, payloadsize, headers, async, freeBuffer, onload, onerror, onprogress) {
    ...
    xhr.addEventListener('load', function (e) {
      if (xhr.status === 200 || _url.substr(0, 4).toLowerCase() !== "http") {
        ...
        if (onload)
			{{{ makeDynCall('viiiii', 'onload') }}} (ctx, buffer, byteArray.length, header_buffer, xhr.status);
        if (freeBuffer) // seems POST reqeusts keeps the buffer
          _free(buffer);
        _free(header_buffer);
      } else {
        if (onerror)
			{{{ makeDynCall('viii', 'onerror') }}} (ctx, xhr.status, xhr.statusText);
      }
    });
    ...
    xhr.addEventListener('progress', function (e) {
      if (onprogress)
	  	{{{ makeDynCall('viii', 'onprogress') }}} (ctx, e.loaded, e.lengthComputable || e.lengthComputable === undefined ? e.total : 0);
    });
...
//    (e||window.event).returnValue = msg; // Gecko + IE
      ...
      for ( var x in callbacks ) {
        ...
        for ( var y in contexts ) {
          try { // jic
			  {{{ makeDynCall('vi', 'callbacks[x].callback') }}} (contexts[y]);
          } catch (e) {}
        }
      }
...
//    return msg; // Gecko + Webkit, Safari, Chrome etc.
  ...
  UE_BrowserWebGLVersion: function() {
	...
	UE_SendJS: function(msg,len) {
		var sdata = Module.HEAP8.subarray(msg, msg+len);

		//uint8 -> string
		var charArray = [];
		for (var i = 0; i < sdata.length; i++) {
			charArray.push(String.fromCharCode(sdata[i]));
		}
		var dataString = charArray.join('');
		// var dataString = "";
		// for (var i = 0; i < sdata.length; i++) {
		...
		try {
			// 解码操作添加异常处理
			sendBuff = decodeURIComponent(window.atob(dataString));
			console.log(sendBuff);
		} catch (decodeError) {
			console.log('解码过程中出现错误:', decodeError);
			return;
		}

	  try {
			let megjson = JSON.parse(sendBuff);
		  let msgevent = new CustomEvent(String(sendBuff.Command), {detail: {message:sendBuff.Message}});
		  window.dispatchEvent(msgevent);
	  }catch (e) {
		  console.log(e);
	  }
	},
	UE_EngineRegisterMessageListener:function(listener){
		UE_JSlib.UE_Recemsg = function(msg) {
			{{{ makeDynCall('vp', 'listener') }}} (msg);
		}
	}
